...
<style>
  ...
  .nutrient-card {
    background: #1B6CA8; color: white; border-radius: 10px;
    padding: 12px 16px; margin: 4px 0;
    border-left: 4px solid #0d4a7a;
  }
  .nutrient-card b { color: #cfe8ff; font-size: 0.82em; text-transform: uppercase; letter-spacing: 0.04em; }
  .nutrient-card.warning { background: #B71C1C; border-left-color: #7f1111; color: white; }
  .nutrient-card.warning b { color: #ffcdd2; }
  .stat-box {
...
with tab1:
    ...
    # ════════════════════════════════════════════════════════════════════════
        ...
        if "error" not in health and health:
            ...
            for dim, score in health["dimension_scores"].items():
                if score is not None:
                    ...
                    st.markdown(
                        f'<div style="margin:12px 0;padding:14px;background:{bar_col};border-radius:10px;color:white">'
                        f'<div style="display:flex;justify-content:space-between;align-items:center;color:white">'
                        f'<b>{dim}</b><span style="font-size:1.2em;font-weight:bold;color:white">{score}/10</span></div>'
                        f'<div style="background:rgba(255,255,255,0.3);border-radius:6px;height:10px;margin:6px 0">'
                        f'<div style="background:rgba(255,255,255,0.85);width:{bar_w}%;height:10px;border-radius:6px"></div></div>'
                        f'</div>',
                        unsafe_allow_html=True
                    )
                    st.markdown(f'<div style="font-size:0.82em;color:#555;margin:-8px 0 8px 0;padding:0 4px">{dim_explanations.get(dim,"")}</div>', unsafe_allow_html=True)
        ...
        with wn1:
            st.markdown(
                '<div style="background:#2E7D32;color:white;border-radius:12px;padding:16px;text-align:center">'
                '<div style="font-size:2em">📸</div>'
                '<b>Scan another food</b><br>'
                '<small style="color:rgba(255,255,255,0.85)">Keep building the database — every food counts!</small>'
        ...
        with wn2:
            st.markdown(
                '<div style="background:#1B6CA8;color:white;border-radius:12px;padding:16px;text-align:center">'
                '<div style="font-size:2em">📊</div>'
                '<b>View the database</b><br>'
                '<small style="color:rgba(255,255,255,0.85)">See all foods scanned by the team so far</small>'
                '</div>', unsafe_allow_html=True
            )
        ...
        with wn3:
            st.markdown(
                '<div style="background:#E65100;color:white;border-radius:12px;padding:16px;text-align:center">'
                '<div style="font-size:2em">📋</div>'
                '<b>Complete this entry</b><br>'
                '<small style="color:rgba(255,255,255,0.85)">Open Google Sheet to fill in fibre, micronutrients & verify NOVA</small>'
                '</div>', unsafe_allow_html=True
            )
